...
const fetchNotifications = async () => {
  loading.value = true
  try {
    const { data } = await axios.get('/coach/notifications')
...
const markAsRead = async (id, e) => {
  e?.stopPropagation()
  try {
    await axios.post(`/coach/notifications/${id}/read`)
...
const markAllAsRead = async () => {
  try {
    await axios.post('/coach/notifications/mark-all-read')
    notifications.value = []
    hasNewNotifications.value = false
  } catch (error) {
    console.error(error)
