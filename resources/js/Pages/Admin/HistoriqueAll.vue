...
const filterOptions = [
  ...
  { key: 'delete_reservation', label: 'Annulations' }
]
...
const stats = computed(() => {
  return {
    ...
    reservations: historiques.value.filter(h => h.action === 'reservation').length,
    annulations: historiques.value.filter(h => h.action === 'delete_reservation').length
  }
})
...
const getActionIcon = (action) => {
  const icons = {
    ...
    update: Edit,
    delete: Trash2,
    reservation: Calendar, // Icône calendrier vert pour réservation
    delete_reservation: CalendarX // Icône calendrier barré rouge pour annulation
  }
...
const getActionColor = (action) => {
  const colors = {
    ...
    reservation: { bg: 'bg-green-500/10', text: 'text-green-400' }, // Vert pour réservation
    delete_reservation: { bg: 'bg-red-500/10', text: 'text-red-400' } // Rouge pour annulation
  }
...
const markAsRead = async (id) => {
  try {
    await axios.post(`/coach/historique/${id}/read`) 
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-lg font-bold text-white">{{ historiques.length }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div class="flex items-center">
            <div class="p-2 bg-emerald-500/10 rounded-lg mr-3">
              <Plus class="w-5 h-5 text-emerald-400" />
            </div>
            <div>
              <p class="text-xs text-gray-400">Ajouts</p>
              <p class="text-lg font-bold text-white">{{ stats.ajouts }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div class="flex items-center">
            <div class="p-2 bg-amber-500/10 rounded-lg mr-3">
              <Edit class="w-5 h-5 text-amber-400" />
            </div>
            <div>
              <p class="text-xs text-gray-400">Modifications</p>
              <p class="text-lg font-bold text-white">{{ stats.modifications }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-lg font-bold text-white">{{ stats.reservations }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div class="flex items-center">
            <div class="p-2 bg-red-500/10 rounded-lg mr-3">
              <CalendarX class="w-5 h-5 text-red-400" />
            </div>
            <div>
              <p class="text-xs text-gray-400">Annulations</p>
              <p class="text-lg font-bold text-white">{{ stats.annulations }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
          ...
          >
            <div class="grid grid-cols-12 gap-4 items-center">
              ...
              <div class="col-span-2 text-center">
                <button
                  v-if="!historique.is_read"
                  @click="markAsRead(historique.id)"
                  :class="[
                    'inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-all cursor-pointer',
        ...
        <div v-if="filteredHistoriques.length === 0" class="text-center py-12">
          ...
          <p class="text-gray-400">Aucune activité récente à afficher pour ce filtre.</p>
        </div>
      </div>
    </div>
  </div>
